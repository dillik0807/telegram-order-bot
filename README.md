# Telegram Order Bot → WhatsApp

Telegram-бот для приема заявок от клиентов с автоматической отправкой в WhatsApp.

## 🚀 Быстрый старт

### 1. Установка зависимостей

```bash
cd telegram-order-bot
npm install
```

### 2. Настройка

Скопируйте `.env.example` в `.env`:

```bash
copy .env.example .env
```

### 3. Получение Telegram Bot Token

1. Откройте Telegram и найдите [@BotFather](https://t.me/BotFather)
2. Отправьте команду `/newbot`
3. Следуйте инструкциям (имя бота, username)
4. Скопируйте полученный токен в `.env` файл

```env
TELEGRAM_BOT_TOKEN=1234567890:ABCdefGHIjklMNOpqrsTUVwxyz
```

### 4. Настройка WhatsApp (выберите один вариант)

#### ✅ Вариант 1: Meta WhatsApp Cloud API (рекомендуется)

1. Зарегистрируйтесь на [Meta for Developers](https://developers.facebook.com/)
2. Создайте приложение WhatsApp Business
3. Получите `Phone Number ID` и `Access Token`
4. Добавьте в `.env`:

```env
WHATSAPP_PHONE_ID=your_phone_id
WHATSAPP_ACCESS_TOKEN=your_access_token
WHATSAPP_RECIPIENT=+992900000000
```

#### ✅ Вариант 2: Twilio

1. Зарегистрируйтесь на [Twilio](https://www.twilio.com/)
2. Получите WhatsApp Sandbox или Business номер
3. Добавьте в `.env`:

```env
TWILIO_ACCOUNT_SID=your_account_sid
TWILIO_AUTH_TOKEN=your_auth_token
TWILIO_WHATSAPP_FROM=whatsapp:+14155238886
TWILIO_WHATSAPP_TO=whatsapp:+992900000000
```

#### ✅ Вариант 3: Telegram группа (самый простой для старта)

1. Создайте группу в Telegram
2. Добавьте бота в группу как администратора
3. Получите ID группы (отправьте сообщение в группу и используйте [@userinfobot](https://t.me/userinfobot))
4. Добавьте в `.env`:

```env
TELEGRAM_GROUP_ID=-1001234567890
```

### 5. Запуск бота

```bash
npm start
```

Для разработки с автоперезагрузкой:

```bash
npm run dev
```

## 📋 Как работает бот

1. Клиент отправляет `/start`
2. Бот запрашивает:
   - Имя клиента
   - Телефон
   - Склад (из списка)
   - Товары (можно добавить несколько)
   - Количество каждого товара
   - Номер транспорта
   - Комментарий (опционально)
3. Клиент подтверждает заявку
4. Заявка сохраняется в БД (SQLite)
5. Заявка отправляется в WhatsApp или Telegram группу

## 📦 Пример заявки

```
📦 НОВАЯ ЗАЯВКА

👤 Клиент: Иван
📞 Телефон: +992900000000
🏬 Склад: Склад №1

🛒 Товары:
1) Цемент — 10 шт
2) Песок — 5 т

🚚 Транспорт: 1234 AB
📝 Комментарий: Срочно

⏰ Время: 16.01.2026, 15:30:00
```

## 🗄️ База данных

Используется SQLite с тремя таблицами:

- `users` - пользователи
- `orders` - заявки
- `order_items` - товары в заявках

База создается автоматически при первом запуске.

## 🛠️ Команды бота

- `/start` - начать новую заявку
- `/cancel` - отменить текущую заявку

## 📝 Настройка под себя

### Изменить список складов

В файле `bot.js` найдите:

```javascript
const warehouses = ['Склад №1', 'Склад №2', 'Склад №3', 'Другой'];
```

### Изменить список товаров

```javascript
const products = [
  'Цемент',
  'Песок',
  'Щебень',
  'Кирпич',
  'Арматура',
  'Другое'
];
```

## 🔧 Troubleshooting

### Бот не отвечает

- Проверьте правильность токена в `.env`
- Убедитесь, что бот запущен (`npm start`)

### WhatsApp не работает

- Проверьте настройки API в `.env`
- Используйте вариант с Telegram группой для тестирования

### Ошибка базы данных

- Удалите файл `orders.db` и перезапустите бота

## 📚 Дополнительно

### Деплой на сервер

Для продакшена рекомендуется:

1. Использовать PM2 для управления процессом
2. Настроить PostgreSQL вместо SQLite
3. Добавить логирование
4. Настроить webhook вместо polling

### Расширение функционала

Можно добавить:

- Админ-панель для просмотра заявок
- Статистику по заявкам
- Уведомления менеджерам
- Экспорт в Excel
- Интеграцию с CRM

## 📞 Поддержка

Если возникли вопросы - создайте issue или свяжитесь с разработчиком.
