...
bot.on('text', async (ctx) => {
  ...
  try {
    ...
    if (data.step === 'add_more') {
      ...
      if (text === '✅ Продолжить') {
        // 🔧 ИСПРАВЛЕНИЕ: Администратор всегда вводит данные клиента заново
        const isAdminUser = admin.isAdmin(userId);
        
        if (isAdminUser) {
          // Администратор создает заявку для клиента - всегда запрашиваем данные
          data.step = 'name';
          orderData.set(userId, data);
    ...
    if (data.step === 'name') {
      ...
      // 🔧 ИСПРАВЛЕНИЕ: Администратор не может использовать "-"
      const isAdminUser = admin.isAdmin(userId);
      
      // Если пользователь отправил "-", используем существующее имя (только для обычных клиентов)
      if (text === '-') {
        if (isAdminUser) {
      ...
      // Для администратора - всегда запрашиваем телефон клиента
      if (isAdminUser) {
        return ctx.reply('📞 Введите номер телефона клиента:\n(например: +992900000000)');
      }
      
      // Для обычных клиентов - проверяем, есть ли сохраненный телефон
      const client = await database.getClient(userId);
    ...
    if (data.step === 'phone') {
      ...
      // Если пользователь отправил "-", используем существующий телефон (только для обычных клиентов)
      if (text === '-') {
        if (isAdminUser) {
      ...
      if (!isAdminUser) {
        // Сохраняем имя и телефон в базу данных только для обычных клиентов
        try {
